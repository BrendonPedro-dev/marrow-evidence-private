...
Sections 1-5 above describe the table as it stood at 131 rows and are left exactly as written.
The re-sweep added rows 132-146 to `campaigns.md` under the identical definition, pinned edge cases and window; see the "Re-sweep 2026-07" method note at the foot of that file for the query class, the anti-inflation checks, and the excluded/logged candidates.

### The numbers

| | Before (n=131) | After (n=146) |
|---|---|---|
| PORTFOLIO | 90 (68.7%) | 94 (64.4%) |
| INDEPENDENT | 40 (**30.5%**) | 51 (**34.9%**) |
| UNCLEAR | 1 (0.8%) | 1 (0.7%) |

**Indie share: 30.5% -> 34.9% (+4.4pts).**

Per year:

| Year | n before | indie before | n after | indie after |
|---|---|---|---|---|
| 2024 | 19 | 47.4% | 19 | 47.4% (unchanged - no 2024 rows found) |
| 2025 | 90 | 26.7% | 99 | **32.3%** |
| 2026YTD | 22 | 31.8% | 28 | **35.7%** |

**Rows added: 15 - 11 INDEPENDENT, 4 PORTFOLIO.**
The four portfolio rows were appended deliberately: the artist-side net catches them, and dropping them would have manufactured a bigger correction than the evidence supports.

Addressable number: REPRESENTABLE=yes on indie rows moves **21 -> 26**, so the addressable-indie share moves **16.0% -> 17.8%** of all campaigns.
The five new REP=yes rows are NLLE (row 133), イコモチ (134), mikko (136), the Pixio four-illustrator roster (140) and 寺田てら (141); the six new REP=unclear rows are くらはしれい (132, Gakken co-©), しかるねこ (135), しぐれうい (137), the two 可哀想に! apparel rows (138, 139) and mame (142, agency-repped).

### Contradicted prior calls

**None.** No row 1-131 was reclassified, re-dated or edited by this pass. Nothing the artist-side net returned contradicted a recorded call; where a new row touches an existing licensor relationship (可哀想に!, しぐれうい, Medicos-made goods) the new evidence agreed with the existing class and REPRESENTABLE reading, which is itself a small consistency check on the original pass.

### What the artist-side keywords found that brand-side missed

**A channel, not a hidden creator class.** Every one of the 11 new indie rows is a *retail/venue pop-up, a hardware or service brand's creator programme, or an apparel drop* - formats where the campaign is named after the artist rather than after an IP, so a brand-side query (brand + コラボ, or a named IP) never returns them. The illustrator pop-up circuit in particular (MEDICOS SHOP渋谷, animate池袋, メトロオーパス梅田, キャラコレ渋谷, 松屋銀座 MGカフェ) is a real and reasonably dense Japanese channel that the original sweep touched only through Loft and Village Vanguard.

**Two non-lifestyle-retail formats the original pass had no read on at all:**
- **Hardware/service brands running standing creator-collab programmes** - Pixio is on its 2nd instalment with four named freelance illustrators (row 140), CyberAgent's Pigg Party on its 3rd with 寺田てら (row 141), Superbag on its 4th with mame (row 142). These are recurring, non-exclusive, individually-contracted arrangements - the single most PBC-shaped format found in the whole Japan build.
- **Transport/tourism** - JR東海's 推し旅 built a prefecture-wide tourism campaign around an individual illustrator (row 137). Section 5's format-skew caveat listed no transport vein; it now has one, and it came back indie.

**The bigger finding runs the other way, and it is the honest headline.** The artist-side net's *largest* yield in Japan was not indie campaigns at all - it was portfolio IP paired with a commissioned illustrator (初音ミク x 望月けい, ダンガンロンパ x lack, 東方 x 松本能人, 〈物語〉シリーズ x 望月けい, サンリオ x necömi, and a dozen more, listed in the campaigns.md method note). In Japan the words イラストレーター and 描き下ろし are overwhelmingly used to credit a *hired* artist on someone else's IP. Rows 143 and 144 are that pattern in campaign form and are classed PORTFOLIO on the per-IP ownership test. Row 145 (BLUE HAMHAM, framed as 「有村泰志氏が手掛ける」 but © CHOCOLATE, a multi-IP studio) and row 146 (ナガノマーケット, self-presenting as the creator's own market but portfolio under the pinned Chiikawa rule) are the Japan repeat of Taiwan's dtto-friends finding: **artist-side keywords detect self-presentation, not ownership.** Every hit still has to be run through the ownership test.

### How Japan compares to the two prior legs

Thailand moved +11.2pts (9.3% -> 20.5%), Taiwan +0.4pts (27.1% -> 27.5%), Japan **+4.4pts** (30.5% -> 34.9%).
Japan sits between them and the reason is legible: ja trade press *does* name the illustrator (so the Thai erasure mechanism is weak here, as in Taiwan), but Japan has a large artist-named-campaign channel - the illustrator pop-up circuit and brand creator-programmes - that a brand-side query cannot address by construction. Taiwan's correction was small because its equivalent channel (venue pop-ups) had already been partly swept; Japan's was larger because its equivalent channel is bigger and was barely touched.

### Method limits specific to this pass

- **The pass is a targeted vein, not a random sample.** Querying artist-side terms returns artist-led campaigns; the appended block is indie-skewed by construction (11 of 15). The post-re-sweep 34.9% should be read as *the ratio after adding a channel the first pass under-covered*, not as a random-sample correction of a biased estimate. The direction is right and the mechanism is evidenced; the magnitude is not a confidence interval.
